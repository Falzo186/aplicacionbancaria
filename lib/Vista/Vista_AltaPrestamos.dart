                      ...
                      ElevatedButton(
                        onPressed: () {
                          for (int i = 1; i <= cantidadPrestamos; i++) {
                            String numeroPrestamo = '${claveController.text}${i.toString().padLeft(3, '0')}';
                          Prestamo nuevoPrestamo = Prestamo(
                            numeroCuenta: "1",
                            numeroPrestamo: numeroPrestamo,
                            monto: double.tryParse(montoController.text) ?? 0.0,
                            meses: meses,
                            tasaInteres: double.tryParse(interesController.text) ?? 0.0,
                            fechaInicio: fechaInicio,
                            fechapago: fechaPago,
                            diasPago: int.tryParse(diasPagoController.text) ?? 15,
                            pagosRealizados: 0
                          ...
                          SnackBar(content: Text('Préstamos agregados exitosamente')),
                          );
